...
Michael owns this. Start: say hi Michael + 1 motivating line.
Work style: telegraph; noun-phrases ok; drop grammar; min tokens.

## Agent Protocol
- Contact: Michael Cunningham (@0xascale, mic.b.cunningham@gmail.com).
- Workspace: `~/Projects`. Missing peakyragnar repo: clone `https://github.com/peakyragnar/<repo>.git`.
- 3rd-party/OSS (non-peakyragnar): clone under `~/Projects/oss`.
...
## Important Locations
- Agent loop doc: `docs/agents-loop.md`
- Notes/Runbooks: `~/Projects/manager/docs/` (e.g. `mac-studio.md`, `mac-vm.md`)
- OpenAI/Codex limits tracking: `/Users/michael/usage/codex_limits.md`
- Obsidian vault: `/Users/michael/Obsidian/DevNotes`
...
- Reminder: check `~/.profile` for missing env keys before builds/releases.
...
- Python: use `uv`; run lint/typecheck/tests if configured; avoid new deps w/o check.
- Go: `gofmt`; run `go test ./...`; prefer stdlib; only for hot paths.
...
### bird
- X CLI: `~/Projects/oss/bird/bird`. Cmds: `tweet`, `reply`, `read`, `thread`, `search`, `mentions`, `whoami`.
- Uses Chrome cookies by default (config: `~/.config/bird/config.json5`; override with `--cookie-source`/`--chrome-profile`).

### peekaboo
- Screen tools: `~/Projects/oss/Peekaboo`. Cmds: `capture`, `see`, `click`, `list`, `tools`, `permissions status`.
- Needs Screen Recording + Accessibility. Docs: `~/Projects/oss/Peekaboo/docs/commands/`.

### committer
- Commit helper (PATH at `~/bin/committer`). Stages only listed paths; required here. Repo may also ship `./scripts/committer`.
